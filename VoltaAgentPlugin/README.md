...
3. 동시에 LLM 채팅 세션은 사용자의 프로젝트 브리프, 요청 의도, 다음 액션을 관리한다.
...
AI:
프로젝트에 대해 간단히 설명해주세요.
예: K-pop 발라드 / 여성 보컬 중심 / 기타와 피아노 위주 / 먼저 트랙 정리와 로우컷부터
```
...
4. 서버가 트랙 분석 시작
5. LLM 채팅 세션은 사용자에게 프로젝트 브리프 질문

중요한 점:

- 프로젝트 브리프 질문과 트랙 업로드는 동시에 진행한다.
- 사용자가 브리프를 답하는 동안 서버는 이미 길이, 포맷, 샘플레이트 등의 기초 분석을 진행한다.
...
AI:
프로젝트 분석을 시작하겠습니다.
먼저 프로젝트에 대해 간단히 설명해주세요.

[동시에 JUCE → 서버로 전체 트랙 파일 업로드]

사용자:
K-pop 여성 보컬 곡이고, 기타랑 피아노가 있어요. 우선 트랙 이름 정리하고 로우컷부터 하고 싶어요.

AI:
좋습니다. K-pop 여성 보컬 중심 곡으로 이해했습니다.
먼저 트랙 정리와 로우컷부터 진행할 수 있습니다.
...
```json
{
  ...
  "project_session_id": "proj_123",
  "messages": [],
  "current_step": "collect_project_brief"
}
```
...
현재 JUCE 클라이언트와 Ableton Max for Live 경로는 다음 흐름을 기준으로 정리되었다.

1. Ableton에서 `Volta.amxd`의 `Send`를 눌러 현재 트랙/디바이스/파라미터를 서버로 보낸다.
2. JUCE 클라이언트에서 `에이블톤 정보 불러오기`로 최신 세션이 들어왔는지 확인한다.
3. 같은 화면에서 WAV 폴더를 추가한다.
4. `Analyze WAV Stems`를 눌러 stem 업로드/분석과 session scan 요청을 함께 시작한다.
5. 분석이 끝나면 채팅 패널에서 AI 요청과 action preview를 이어서 진행한다.

즉 사용자는 더 이상 내부 구조를 알 필요 없이 아래 작업 순서만 따르면 된다.

```text
1. Ableton에서 Send
2. JUCE에서 에이블톤 정보 불러오기
3. WAV 폴더 추가
4. Analyze WAV Stems
5. 채팅으로 수정 요청
```
...
- 초기 진입 화면을 채팅 중심이 아니라 **단계 안내 중심 UI**로 재구성
- `Refresh Session` 같은 개발자 용어 대신 `에이블톤 정보 불러오기` 같은 사용자 중심 문구 사용
- WAV 폴더 드래그 앤 드롭 및 폴더 선택 버튼 추가
- `Analyze WAV Stems` 클릭 시 기존 분석 요청과 함께 `POST /request-session-scan` 호출
- 분석 전에는 채팅 입력보다 현재 단계와 준비 상태를 먼저 보여주도록 구조 변경
- M4L 디바이스 안에서도 `준비중... → 전송 시작... → 세션 읽는 중... → 서버로 전송 중... → 전송 완료` 상태를 볼 수 있도록 보조 UI 추가

### 현재 사용자에게 보이는 상태 모델

플러그인 UI는 아래 상태를 기준으로 안내 문구를 바꾼다.

1. `waiting_for_live_session`
2. `live_session_ready`
3. `waiting_for_wav_folder`
4. `ready_to_analyze`
5. `analyzing`
6. `analysis_complete`
7. `ready_for_chat`

이 상태 모델의 목적은 사용자가 "지금 무엇이 준비되었고, 다음에 무엇을 해야 하는지"를 텍스트 한 줄로 바로 이해하게 만드는 것이다.

### 현재 남아 있는 한계

- Ableton 세션 정보의 실제 업로드 주체는 여전히 M4L이다.
- 따라서 `Send`를 누르지 않으면 JUCE가 단독으로 Live 파라미터를 읽어올 수는 없다.
- JUCE의 `request-session-scan`은 M4L polling을 깨우는 트리거일 뿐, 세션 자체를 직접 생성하지 않는다.
- 플러그인 창 유지(`always on top`)는 호스트 정책에 따라 제한될 수 있어, 폴더 선택 버튼 UX를 계속 병행하는 것이 안전하다.

### 이 문서 기준 권장 데모 시나리오

1. Ableton에서 `Volta.amxd`의 상태가 `준비중...`인지 확인
2. 요청 프롬프트를 입력하고 `Send`
3. M4L에서 `전송 시작...`, `세션 읽는 중...`, `서버로 전송 중...`, `전송 완료` 순서 확인
4. JUCE에서 `에이블톤 정보 불러오기`
5. WAV 폴더 추가 후 `Analyze WAV Stems`
6. 분석 완료 후 채팅 요청과 action preview 확인
